...
void loop() {
  if(!goMode){
    if (BTSerial.available()){
      objKey = BTSerial.readStringUntil('\n');
      Serial.println("Now Looking For: " + objKey);
      goMode = true;
      getObjectName(objKey)
    }
  } else{
    ...
    if (irLeft == 1 && irRight == 1){
      ...
      pixy.ccc.getBlocks();
      if(pixy.ccc.numBlocks && (pixy.ccc.blocks[0].m_signature == objectToSearch)){
...
String getObjectName(String objKey) {
  if (objKey == "name1"){
    objectToSearch = 1;
  }
}
